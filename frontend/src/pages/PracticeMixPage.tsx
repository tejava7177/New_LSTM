...
export default function PracticeMixPage() {
  ...
  const [midiAudioUrl, setMidiAudioUrl] = useState<string | null>(null) // 서버에서 받은 WAV URL
  const [midiBuffer, setMidiBuffer] = useState<AudioBuffer | null>(null) // 믹싱용 AudioBuffer
  const [midiTracks, setMidiTracks] = useState<TrackMeta[]>([])
  const [rendering, setRendering] = useState(false)

  // --- 플레이어 레퍼런스 & 상태(각 채널 하나씩만 사용) ---
  ...
  // 언마운트 시 ObjectURL 정리 (mergedUrl만)
  useEffect(() => {
    return () => {
      if (mergedUrl) URL.revokeObjectURL(mergedUrl)
    }
  ...
  return (
    <div className="pmx-wrap">
      ...
      <section className="pmx-panel">
        ...
        {/* 채널별 오디오 (이 엘리먼트만 사용; transport가 이걸 제어) */}
        <div className="mixer">
          <div className="ch">
      ...
      <section className="pmx-panel">
        ...
        {mergedUrl && (
          <div className="result">
            ...
            <div>
              <a className="btn" href={mergedUrl} download={makeDownloadName(midiFile?.name)}>⬇ 합친 결과 다운로드 (WAV)</a>
            </div>
          </div>
        )}
